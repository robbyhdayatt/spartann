<div class="invoice p-3 mb-3 shadow-sm rounded">
    {{-- Header Invoice & Info Pengirim/Penerima --}}
    <div class="row">
        <div class="col-12">
            <h4>
                <i class="fas fa-globe text-primary mr-2"></i> PT. Lautan Teduh Interniaga
                <small class="float-right text-muted" style="font-size: 0.6em;">Date: {{ $purchaseOrder->tanggal_po->format('d/m/Y') }}</small>
            </h4>
        </div>
    </div>

    <div class="row invoice-info mt-4 border-bottom pb-4">
        <div class="col-sm-4 invoice-col">
            <strong class="text-secondary">Diterbitkan Oleh / Dikirim Ke:</strong>
            <address class="mt-2">
        ...
        <div class="col-sm-4 invoice-col">
            <strong class="text-secondary">Kepada Yth (Supplier / Sumber):</strong>
            <address class="mt-2">
                <strong>
        ...
        <div class="col-sm-4 invoice-col">
            <div class="p-3 bg-light rounded">
                <b>Nomor PO:</b> <span class="float-right">#{{ $purchaseOrder->nomor_po }}</span><br>
                <b>Status:</b> 
                <span class="float-right badge {{ $purchaseOrder->status == 'APPROVED' ? 'badge-success' : ($purchaseOrder->status == 'REJECTED' ? 'badge-danger' : 'badge-warning') }}">
                    {{ $purchaseOrder->status }}
                </span>
                @if($purchaseOrder->request_group_id)
                <br><b>Ref Group:</b> <span class="float-right text-muted">{{ $purchaseOrder->request_group_id }}</span>
                @endif
            </div>
        </div>
    </div>

    {{-- ALERT INSTRUKSI UNTUK DEALER REQUEST --}}
    ...
    {{-- FORM WRAPPER (Aktif jika Dealer Request & Pending & User Berhak Approve) --}}
    @php
        $isDealerRequestApproval = ($purchaseOrder->po_type == 'dealer_request' && $purchaseOrder->status == 'PENDING_APPROVAL' && auth()->user()->can('approve-po', $purchaseOrder));
    @endphp
    ...
    {{-- ACTION BUTTONS --}}
    <div class="row no-print mt-5 pt-3 border-top bg-light pb-3">
        <div class="col-12 text-right">
            @if($purchaseOrder->status == 'PENDING_APPROVAL')
                @can('approve-po', $purchaseOrder)
                    
                    {{-- Tombol Reject (Common for both types) --}}
                    <button type="button" class="btn btn-danger mr-2" data-toggle="modal" data-target="#rejectModal">
                        <i class="fas fa-times-circle mr-1"></i> Tolak PO
                    </button>

                    {{-- Tombol Approve: DEALER REQUEST (With Form Submit) --}}
                    @if($isDealerRequestApproval)
                    ...
                    {{-- Tombol Approve: SUPPLIER PO (Direct Form Submit via hidden form - FIX POIN 3) --}}
                    @elseif($purchaseOrder->po_type == 'supplier_po')
                        {{-- Karena tombol di atas dibungkus form khusus dealer request, kita butuh form terpisah --}}
                        <button type="button" class="btn btn-success" onclick="document.getElementById('approve-supplier-form').submit();">
                            <i class="fas fa-check-circle mr-1"></i> Setujui PO Supplier
                        </button>
                    @endif
    ...
    @if($isDealerRequestApproval)
    </form> {{-- Tutup Form Dealer Request --}}
    @endif

    {{-- Hidden Form for Supplier PO Approval --}}
    @if($purchaseOrder->po_type == 'supplier_po' && $purchaseOrder->status == 'PENDING_APPROVAL')
        <form id="approve-supplier-form" action="{{ route('admin.purchase-orders.approve', $purchaseOrder->id) }}" method="POST" style="display: none;">
            @csrf
        </form>
    @endif
</div>

{{-- SCRIPT VALIDASI REAL-TIME (Hanya untuk Dealer Request) --}}
@if($isDealerRequestApproval)
<script>
