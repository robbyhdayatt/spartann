...
class DailyReportImport implements ToCollection
{
    ...
    {
        // Baris pertama dan kedua adalah header, kita skip
        $dataRows = $rows->slice(2);

        // Definisikan header secara manual sesuai hasil debug
        $headers = [
        ...
        foreach ($dataRows as $row) {
            ...
            // Cek apakah ini baris header baru untuk transaksi
            if (!empty($rowData->get('invoice_no')) && !empty($rowData->get('dealer'))) {

                // === KONDISI PENGECEKAN DATA DUPLIKAT ===
                $exists = DailyReport::where('invoice_no', $rowData->get('invoice_no'))
                                     ->where('dealer_code', $rowData->get('dealer'))
                                     ->exists();

                // Jika data sudah ada, lewati baris ini dan set currentReport menjadi null
                if ($exists) {
                    $this->currentReport = null; // Pastikan detail dari baris duplikat tidak ikut tersimpan
                    continue;
                }
                // === AKHIR DARI KONDISI PENGECEKAN ===
                ...
                $this->currentReport = DailyReport::create(
                    [
            ...
            // Jangan proses detail jika tidak ada header transaksi yang aktif
            if (!$this->currentReport) {
                continue;
            }

            // Logika untuk menyimpan detail item (tidak berubah)
            if (!empty($rowData->get('labor_cost_service')) && is_numeric($rowData->get('labor_cost_service'))) {
                $this->currentReport->details()->updateOrCreate(
