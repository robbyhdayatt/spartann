...
class ServiceImport implements OnEachRow, WithChunkReading
{
    ...
    private function normalizeString($value)
    {
        ...
        // 1. Ganti semua varian dash (en-dash, em-dash, dll) dengan dash standar
        $value = str_replace(
            ['–', '—', '−', '‒', '―'],
            '-',
            $value
        ...
        // 2. Ganti non-breaking space (dan varian spasi unicode lainnya) dengan spasi standar
        // \h adalah spasi horizontal, \p{Zs} adalah pemisah spasi unicode
        $value = preg_replace('/[\h\s\p{Zs}]+/u', ' ', $value);

        // 3. Hapus karakter kontrol (non-printable)
        $value = preg_replace('/[\p{C}]/u', '', $value);

        // 4. Trim spasi di awal/akhir
        return trim($value);
    }
    ...
    private function createServiceDetail(Service $service, array $row, int $rowIndex)
    {
        ...
        // Normalisasi nama paket dari Excel
        $servicePackageName = $this->normalizeString($row[$servicePackageName_idx] ?? null);
        $laborCost = $row[$laborCost_idx] ?? null;

        // --- Mulai Logika Jasa ---
        if (!empty($servicePackageName)) {

            // ++ PERBAIKAN: Logika dipisah, tidak lagi menggunakan if-else ++
            ...
            if (isset($this->convertMapping[$servicePackageName])) {
                ...
                $service->details()->create([
                    'item_category' => 'PART', // Sesuai permintaan
                    'service_category_code' => $serviceCategoryCode,
                    'service_package_name' => null, // Sesuai permintaan
            ...
            // 2. Cek JASA BIASA (bukan konversi DAN memiliki biaya)
            elseif (!isset($this->convertMapping[$servicePackageName]) && $laborCost !== null) {
                $service->details()->create([
                    'item_category' => 'JASA',
                    'service_category_code' => $serviceCategoryCode,
                    'service_package_name' => $row[$servicePackageName_idx], // Nama asli dari Excel
                    'item_code' => $servicePackageName,
        ...
        // --- Mulai Logika Part/Oli (Selalu dijalankan) ---
            ...
            if (!empty($invoiceNo)) {
                ...
                if ($service) {
                    ...
                    $this->currentService = Service::create([
                        ...
                        'amount'             => $this->cleanNumeric($row[29] ?? 0),
                ...
                $this->currentServiceCategoryCode = $row[18] ?? null;
